...
U hash_UI(U l, U* v) {
  ...
  else { for (U i=0; i*i<l; i++) h = (i+1000003)**(v+h%l) ^ h<<1; }
  return h;
}
U hash_string(Str v) {
...
U hash_LIST(L l) {
  U h=2184626789L; U n=l->l;
  if (n<256) { DO(i,n) h = (i+1000003)*hash(list_at(l,i)) ^ h<<1; }
  else { for (U i=0; i*i<n; i++) h = (i+1000003)*hash(list_at(l,h%n)) ^ h<<1; }
  return h;
}
...
U hash(V v) {
  ...
  switch (T(v)) {
    ...
    case O_t: case F_t: { O o=O(v); U l = o->l+1; U u[l];
