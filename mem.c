...
V wrapPtr(T t, Ptr p) {
  I s=t_sizeof(t); V v=MALLOC(sizeof(T)+s); *v=t; memcpy(v+1, p, s);
  FREE(p); return v;
}
...
void freePtr(T t, Ptr p) {
  if (PURE(t)) switch (t) {
    ...
    case Q_t: FREE(*(Q*)p); break;
    case O_t: case F_t: { O* o=(O*)p; if (--*o->r) break;
                          FREE(o->r); FREE(o->x); break; }
    case L_t: { L* l=(L*)p; if (--*l->r) break;
                FREE(l->r); FREE(l->p); break; }
  } else {
    freeV(*(V*)p);
  }
}
void freeV(V v) { freePtr(*v, v+1); FREE(v); }
...
V get(V v) {
  if (!(*v&COMP_t) || REF(v)==1) return v;
    ...
    case O_t:
      { O o=O(v); r=makeO(cpy(o.f), o.l, cpyn(o.l, o.x)); FREE(v); break;}
    case F_t:
      { F f=F(v); r=makeF(cpy(f.f), f.l, cpyn(f.l, f.x)); FREE(v); break;}
    case L_t:
      { L l=L(v); I s=t_sizeof(l.t); Ptr p=MALLOC(s*l.c);
        DDO(i,l.l) valcpy(p+i*s, LIST_PTR_ATS(l,i,s), l.t);
        r=makeL(l.t, l.c, l.l, 0, p); FREE(v); break; }
  } return r; }
}
