...
Str Ffmt(F f) {
  ...
  Str ss = toString(f->x[0]); U l=1+strlen(ss);
  DECL_STR(s, l); s[0]=' '; strcpy(s+1, ss); FREE(ss);
  if (f->l==1) {
    ss = toString(f->f); U e=strlen(ss); s=realloc(s,l+3+e);
    s[l]='.'; strcpy(s+l+1, ss); s[l+1+e]=' '; s[l+2+e]='\0';
    FREE(ss); return s;
  } else if (f->l==2) {
    ss = toString(f->f); U lf=strlen(ss);
    Str sr = toString(f->x[1]); U lr=strlen(sr);
...
Str Ofmt(O o) {
  ...
  Str ss = toString(o->x[0]); U l=1+strlen(ss);
  DECL_STR(s, l); s[0]=' '; strcpy(s+1, ss); FREE(ss);
  if (o->l==1) {
    ss = toString(o->f); U e=strlen(ss); s=realloc(s,l+3+e);
    s[l]='o'; strcpy(s+l+1, ss); s[l+1+e]=' '; s[l+2+e]='\0';
    FREE(ss); return s;
  } else if (o->l==2) {
    ss = toString(o->f); U lf=strlen(ss);
    Str sr = toString(o->x[1]); U lr=strlen(sr);
    s=realloc(s,l+lf+lr+2);
    strcpy(s+l, ss); strcpy(s+l+lf, sr); strcpy(s+l+lf+lr, " ");
    FREE(ss); FREE(sr); return s;
  }
...
Str PShowI(T t, P p, U indent);
Str ShowI(V v, U indent);
Str TShow(T t) {
  switch (t) {
